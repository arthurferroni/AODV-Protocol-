...
bool cBuffer::insert(cMessage* m)
{
	if( (used + (int)m->length()) > capacity)
		return false;
	//make a copy of the message we are goin to store
	used += m->length();
...
void SimpleMac::handleMessage(cMessage* msg)
{
	...
	if (msg == endService)
	{
		...
		if(!buffer->empty())
		{
			...
			send(m, "toRoute"); // FIXME it is illegal to send a msg object and keep referencing it!!! -Andras
	...
	else
	{
		...
		else
		{
			...
			if( (!routerBusy) && (buffer->empty()) )
			{
				...
				send(msg,"toRoute"); // FIXME it is illegal to send a msg object and keep referencing it!!! -Andras
...
void SimpleMac::finish()
{
	...
	fprintf(fout,"Buffer full discrds...........  %d\n",bufferFullDiscard);
 	fclose(fout);

	d("Buffer full discrds...."<<bufferFullDiscard);
}
