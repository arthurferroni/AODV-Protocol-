...
{
	hosts.setName("hosts");
	nbList.setName("nblist");
	hosts.takeOwnership(false); // FIXME this is a terrible hack to prevent crash on exit because of double deletion --Andras
...
int Physic::connectModules(int rPhysic,int rMod)
{
	...
	cBasicChannel *channel = new cSimpleChannel("channel");

	channel->setDelay(delay->doubleValue());
	channel->setDatarate(dataRate->doubleValue());
	channel->setError(error->doubleValue());
...
bool Physic::isReachable(int from, int to)
{
	...
	//Pitagora @ work...
	dist = sqrt( (double) ((x - s->posX ) * (x - s->posX )) +
		    ((y - s->posY ) * (y - s->posY )) );

	a = sqrt( (double) ((d->posX - s->posX ) * (d->posX - s->posX )) +
		   ((d->posY - s->posY ) * (d->posY - s->posY )) );
	r = ( dist <= range);
	return r;
}
