...
void AODV::broadcast(cMessage* reply)
{
	if(reply !=NULL)
	{
		...
		if( ttl >= 0 )
		{
			...
			send(reply,"toMac"); // FIXME it is illegal to send a msg object and keep referencing it!!! -Andras
...
cMessage* AODV::sendData(cMessage* msg)
{
	...
	if( (e == NULL) || (e->active==false))
	{
		...
		//bufferize the request while look for a route,
		//returns the event that has to be scheduled to
		//menage the retrasmit RREQ event
...
cMessage* AODV::bufferize(int dest,int pktSize)
{
	...
	//this is a new paket : create the message that
	//make the RREQ msg to be timed-out and reseded
	p = new WaitingPkt;
	...
	for(cQueue::Iterator iter(pktBuffer,1); !iter.end(); iter++)
	{
		...
		if(p->dest == (int) msg->par("dest"))
		{
			...
			else
			{
				...
				//increse the TTL value. If this is bigger than a TTL_Threshold
...
cMessage* AODV::handleDelete(cMessage* msg)
{
	...
	if(e->active)
	{
		...
		//route has exired, set it invalid
...
cMessage* AODV::handleData(cMessage* msg)
{
	...
	{
		//this host in not the final destination but the message states that
